...
class SQLquery {
    ...
    public function selectFacuty(){
        try{
            $stmt = $this->conn->query("SELECT f.id AS fid,f.facuty ,f.major_subject AS f_major, facuty, d.location AS location ,CASE WHEN d.location IS NULL THEN 0 ELSE COUNT(*) END AS total  FROM facuty f LEFT JOIN detail d  ON d.facuty_id = f.id GROUP BY f.id");
            $result = $stmt-> fetchALL(PDO::FETCH_ASSOC);
            return $result;
        }
        catch(PDOException $e) {
    ...
    public function selectCoordinate(){
        try{
            $stmt = $this->conn->prepare("SELECT d.id,d.location,f.id AS fid, f.facuty,
    ...
    public function selectAllDetail($id,$region,$province){
        try {
            ...
            $sql = "SELECT d.id,d.location, d.department, d.Scope_work, d.receive_term1, d.receive_term2,
                    f.id AS fid, f.major_subject AS majorName, f.facuty, r.name AS regionName, r.id AS rid,
    ...
    public function selectToMap($region,$province,$facuty,$major_subject){
        ...
        if (isset($facuty) && $facuty != "" ) {
            // ถ้ามีเงื่อนไขก่อนหน้า ใช้ AND ต่อ ไม่งั้นใช้ WHERE
            $mainWordQuery .= (empty($mainWordQuery) ? " WHERE" : " AND") . " f.facuty = ". "'$facuty'";
            
        }

        if (isset($major_subject) && $major_subject != "" ) {
            $mainWordQuery .= (empty($mainWordQuery) ? " WHERE" : " AND") . " f.major_subject =  " . "'$major_subject'";
        }
        // เพิ่มเงื่อนไขเพิ่มเติมในคำสั่ง SQL
